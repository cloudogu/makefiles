...
K8S_HELM_CRD_RELEASE_TGZ=${K8S_HELM_CRD_TARGET}/${ARTIFACT_ID}-crd-${VERSION}.tgz
...
##@ K8s - Component CRD targets

.PHONY: helm-generate-crd-chart
helm-generate-crd-chart: ${K8S_HELM_CRD_TARGET}/Chart.yaml

.PHONY: ${K8S_HELM_CRD_TARGET}/Chart.yaml
${K8S_HELM_CRD_TARGET}/Chart.yaml: ${BINARY_YQ} $(K8S_RESOURCE_TEMP_FOLDER) k8s-generate
	...
	@${BINARY_YQ} 'select(.kind == "CustomResourceDefinition")' $(K8S_RESOURCE_TEMP_YAML) > ${K8S_HELM_CRD_TARGET}/templates/crd.yaml
...
.PHONY: helm-package-crd
helm-package-crd: ${BINARY_HELM} helm-delete-existing-crd-tgz ${K8S_HELM_CRD_RELEASE_TGZ} ## Generates and packages the helm crd-chart.

.PHONY: helm-delete-existing-crd-tgz
helm-delete-existing-crd-tgz: ## Remove an existing Helm package.
# remove
	@rm -f ${K8S_HELM_CRD_RELEASE_TGZ}*

${K8S_HELM_CRD_RELEASE_TGZ}: ${BINARY_HELM} helm-generate-crd-chart $(K8S_POST_GENERATE_TARGETS) ## Generates and packages the helm crd-chart.
